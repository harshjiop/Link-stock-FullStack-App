...
const verifyJWT = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    const decodedToken = await jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (async (error, decodes) => {
      // check wether token expired or not
      if (error?.name === 'TokenExpiredError') {
        // getting details by decoding access token(by ignoring expiration)
        const expiredToken = jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, { ignoreExpiration: true })
        ...
        const newDecodedToken = jwt.verify(newAccessToken, process.env.ACCESS_TOKEN_SECRET)
